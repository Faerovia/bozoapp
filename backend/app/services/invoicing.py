...
async def compute_amount_for_tenant(
    ...
    if tenant.billing_type == "monthly":
        items = [{
            "description": f"Předplatné BOZOapp — {period_label}",
            "quantity": 1,
            "unit": "měsíc",
    ...
    if tenant.billing_type == "yearly":
        ...
        if tenant.created_at.month != period_from.month:
            return Decimal("0"), []
        items = [{
            "description": f"Roční předplatné BOZOapp — {period_from.year}",
            "quantity": 1,
            "unit": "rok",
            "unit_price": float(amount),
            "total": float(amount),
    ...
    if tenant.billing_type == "per_employee":
        ...
        items = [{
            "description": f"BOZOapp za zaměstnance — {period_label}",
            "quantity": count,
            "unit": "ks",
            "unit_price": float(amount),
            "total": float(total),
    ...
    if tenant.billing_type == "custom":
        items = [{
            "description": (
                tenant.billing_note
                or f"Služby BOZOapp — {period_label}"
            ),
            "quantity": 1,
            "unit": "ks",
            "unit_price": float(amount),
